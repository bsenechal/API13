...
public class ServerDataToCommImplTest {
    ...
    /**
     * Test if the disconnect function remove a PublicUserEntity from
     * currentUsers correctly
     */
    @Test
    public void disconnectTest() {
        ...
        Assert.assertEquals("CurrentUsers should contain " + nbRandomUser + " items", nbRandomUser,
                dataServerManager.getCurrentUsers().size());
        Assert.assertFalse("CurrentUsers shouldn't contain the user with the id : " + idUser,
                dataServerManager.getCurrentUsers().contains(idUser));
    }
    ...
    public void getUserInfoTest() {
        ...
        Assert.assertNotNull("DataServerManager shouldn't be null", dataServerManager);
        Assert.assertNotNull("CurrentUsers shouldn't be null", dataServerManager.getCurrentUsers());
        Assert.assertEquals("CurrentUsers should contain " + (nbRandomUser + 1) + " items", (nbRandomUser + 1),
                dataServerManager.getCurrentUsers().size());
        Assert.assertNotNull("getUserInfo shouldn't be null for user : " + idUser,
                dataServerManager.getServerDataToComImpl().getUserInfo(idUser));
        Assert.assertEquals("getUserInfo().Login should be " + login, login,
                dataServerManager.getServerDataToComImpl().getUserInfo(idUser).getLogin());
        Assert.assertEquals("getUserInfo().LastName should be " + lastname, lastname,
                dataServerManager.getServerDataToComImpl().getUserInfo(idUser).getLastName());
        Assert.assertFalse("CurrentUsers should contain the user with the id : " + idUser,
                dataServerManager.getCurrentUsers().contains(idUser));
    }
    ...
    public void notifyConnectionsTest() {
        ...
        Assert.assertTrue("CurrentUsers list should contains " + user.toString(),
                dataServerManager.getCurrentUsers().contains(user));
    }
    ...
    public void getConnectedUsersTest() {
        ...
        Assert.assertNotNull("DataServerManager shouldn't be null", dataServerManager);
        Assert.assertNotNull("CurrentUsers shouldn't be null",
                dataServerManager.getServerDataToComImpl().getConnectedUsers());
        Assert.assertEquals("CurrentUsers should contain " + nbRandomUser + " items", nbRandomUser,
                dataServerManager.getServerDataToComImpl().getConnectedUsers().size());
    }
    ...
    public void getUsersByGameTest() {
        ...
        Assert.assertNull("The result should be null",
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame));

        dataServerManager.getCurrentGames().add(game);

        game.setWhitePlayer(whitePlayer);
        Assert.assertNotNull("The result shouldn't be null",
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame));
        Assert.assertEquals("The list size should be equals to 1", 1,
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).size());
        Assert.assertTrue("The list should contain the user " + whitePlayer.toString(),
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).contains(whitePlayer));

        game.setBlackPlayer(blackPlayer);
        Assert.assertNotNull("The result shouldn't be null",
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame));
        Assert.assertEquals("The list size should be equals to 2", 2,
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).size());
        Assert.assertTrue("The list should contain the user " + blackPlayer.toString(),
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).contains(blackPlayer));
        Assert.assertTrue("The list should contain the user " + whitePlayer.toString(),
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).contains(whitePlayer));
        ...
        Assert.assertNotNull("The result shouldn't be null",
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame));
        Assert.assertEquals("The list size should be equals to 4", 4,
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).size());
        Assert.assertTrue("The list should contain the user " + blackPlayer.toString(),
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).contains(blackPlayer));
        Assert.assertTrue("The list should contain the user " + whitePlayer.toString(),
                dataServerManager.getServerDataToComImpl().getUsersByGame(idGame).contains(whitePlayer));
        Assert.assertTrue("The list should contain the user " + observer1.toString(),
