...
public class UserService {
    ...
    /**
     * Delete the given user from files To use only if the user's login hasn't
    ...
    /**
     * Delete the user with id userId from the files Works in any case
     * 
     * @param userId
    ...
    public void importProfile(final File file, boolean force) throws FunctionalException, TechnicalException {
        ...
        // throw an exception when the user exists already and the force mode is
        // not activated
    ...
    public File exportProfile(PrivateUserEntity user) throws TechnicalException {
        URL source = UserService.class.getClassLoader()
                .getResource("user/" + user.getLogin() + "_" + user.getId() + ".ser");
